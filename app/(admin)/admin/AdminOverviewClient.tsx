...
import { useRealtimeTable, usePresenceBreakdown } from "@/lib/realtime/hooks";
...
export default function AdminOverviewClient({ initialData }: { initialData: AdminStatsOverview | null }) {
  const [data, setData] = useState<AdminStatsOverview | null>(initialData);
  ...
  // Live global presence — read-only view, admin passively tracks with role
  const presence = usePresenceBreakdown(
  ...
  const engagementSeries = useMemo(() => {
    if (!data) return [];
    const byDay = new Map<string, { day: string; impressions: number; whatsapp: number; messages: number }>();
    const put = (key: "impressions" | "whatsapp" | "messages", series?: Array<{ day: string; count: number }>) => {
      for (const p of series ?? []) {
        const row = byDay.get(p.day) ?? { day: p.day, impressions: 0, whatsapp: 0, messages: 0 };
        row[key] = p.count;
        byDay.set(p.day, row);
      }
    };
    put("impressions", data.trends.impressions);
    put("whatsapp", data.trends.whatsapp);
    put("messages", data.trends.messages);
    return Array.from(byDay.values()).sort((a, b) => a.day.localeCompare(b.day));
  }, [data]);
  ...
  if (error) {
    return (
      <div className="space-y-4">
        <Header onRefresh={() => load()} refreshing={refreshing} generated={null} />
  ...
  if (!data) {
    return (
      <div className="space-y-4">
        <Header onRefresh={() => load()} refreshing={refreshing} generated={null} />
        <div className="dm-card p-8 text-sm text-muted">No stats available.</div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6 sm:space-y-8">
      ...
      <section className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5">
        ...
        <KpiCard
          label="Impressions"
          value={fmt(s.total_impressions ?? 0)}
          sub={`${fmt(s.impressions_last_24h ?? 0)} last 24h`}
          accent={PALETTE[3]}
        />
        <KpiCard
          label="WhatsApp (window)"
          value={fmt(s.whatsapp_clicks_window ?? 0)}
          sub={`${fmt(s.whatsapp_clicks_today ?? 0)} today`}
          accent={PALETTE[9]}
        />
        <KpiCard
          label="Users"
      ...
      <section className="dm-card p-5 sm:p-6">
        <div className="flex flex-wrap items-end justify-between gap-3">
          <div>
            <h2 className="font-display text-lg font-semibold tracking-tight">Live presence</h2>
            <p className="mt-1 text-xs text-muted sm:text-sm">
              Real-time count of connected browsers on Midora (via Supabase Realtime).
            </p>
          </div>
          <div className="flex items-center gap-1 rounded-full border border-emerald-100 bg-emerald-50 px-2.5 py-1 text-[11px] font-bold text-emerald-700">
        ...
        <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          {[
            { label: "Online now",       value: presence.total,             color: PALETTE[0] },
            { label: "Signed in",        value: presence.authenticated,     color: PALETTE[1] },
            { label: "Guests",           value: presence.guests,            color: PALETTE[3] },
            { label: "Merchants online", value: presence.merchants,         color: PALETTE[4], sub: `${presence.availableMerchants} available` },
      ...
      {/* Engagement trend (impressions / whatsapp / messages) */}
      <section className="dm-card p-5 sm:p-6 lg:p-8">
        <div>
          <h2 className="font-display text-lg font-semibold tracking-tight">
            Engagement trend
          ...
          <p className="mt-1 text-xs text-muted sm:text-sm">
            Daily impressions, WhatsApp clicks, and messages across the platform
            over the last {data.window_days} days.
          </p>
        </div>
        <div className="mt-6 h-72 w-full sm:h-80">
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={engagementSeries} margin={{ top: 8, right: 16, left: -8, bottom: 0 }}>
              <defs>
                {(["impressions", "whatsapp", "messages"] as const).map((k, i) => (
                  <linearGradient key={k} id={`eng-grad-${k}`} x1="0" y1="0" x2="0" y2="1">
                    <stop offset="0%" stopColor={PALETTE[i + 3]} stopOpacity={0.35} />
                    <stop offset="100%" stopColor={PALETTE[i + 3]} stopOpacity={0.02} />
                  </linearGradient>
                ))}
              </defs>
              <CartesianGrid strokeDasharray="3 3" stroke={CARTESIAN_STROKE} />
              <XAxis dataKey="day" tick={{ fontSize: 11 }} stroke={AXIS_STROKE} tickFormatter={formatDay} minTickGap={32} />
              <YAxis tick={{ fontSize: 11 }} allowDecimals={false} stroke={AXIS_STROKE} />
              <Tooltip
                labelFormatter={(v: string) => formatDay(v)}
                contentStyle={{ borderRadius: 12, border: "1px solid rgba(102, 121, 143, 0.25)", backgroundColor: "rgba(255,255,255,0.98)", fontSize: 12 }}
              />
              <Legend wrapperStyle={{ fontSize: 12 }} iconType="circle" />
              {(["impressions", "whatsapp", "messages"] as const).map((k, i) => (
                <Area
      ...
      {/* Impression pool mix */}
      {data.distributions.impression_pools && data.distributions.impression_pools.length > 0 ? (
        <section className="dm-card p-5 sm:p-6">
          <h2 className="font-display text-lg font-semibold tracking-tight">
            Impression sources
          </h2>
          <p className="mt-1 text-xs text-muted sm:text-sm">
            Distribution of impressions across feed composition pools.
          </p>
          <DistributionChart
            slices={data.distributions.impression_pools}
            emptyLabel="No impressions yet"
          />
        </section>
      ) : null}
              ...
              <tbody>
                {data.top_shops.map((shop, i) => (
                  <tr key={shop.id} className="rounded-xl bg-foreground/[0.03]">
                    ...
                    <td className="px-3 py-3 text-right tabular-nums">{fmt(shop.impressions ?? 0)}</td>
