...
type Props = {
  initialFollowed: number;
  initialLiked: number;
};

export default function CustomerOverviewClient({ initialFollowed, initialLiked }: Props) {
  const session = useAppSession();
  const firstName = session.user?.full_name?.trim().split(" ")[0]
    || session.user?.email?.trim()
    || "there";

  const [followed, setFollowed] = useState<number>(initialFollowed);
  const [liked, setLiked] = useState<number>(initialLiked);

  // Client-side refresh once hydrated — SSR provided the initial counts.
  useEffect(() => {
    ...
    <div className="space-y-6 sm:space-y-8">
      ...
      <section className="grid gap-3 sm:grid-cols-3">
        <Link href="/customer/saved" className="dm-card group flex flex-col gap-2 p-5 transition hover:-translate-y-0.5">
          ...
          <p className="font-display text-3xl font-semibold text-accent">{followed}</p>
        ...
        <Link href="/customer/saved" className="dm-card group flex flex-col gap-2 p-5 transition hover:-translate-y-0.5">
          ...
          <p className="font-display text-3xl font-semibold" style={{ color: "#c17767" }}>{liked}</p>
